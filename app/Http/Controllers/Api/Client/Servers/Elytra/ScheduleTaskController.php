...
class ScheduleTaskController extends ClientApiController
{
    ...
    public function store(StoreTaskRequest $request, Server $server, Schedule $schedule): array
    {
        ...
        if ($schedule->tasks()->count() >= $limit) {
            throw new ServiceLimitExceededException(trans('exceptions.tasks.schedule_task_limit', ['limit' => $limit]));
        }

        if (!$server->allowsBackups() && $request->action === 'backup') {
            throw new HttpForbiddenException(trans('exceptions.tasks.backup_disabled'));
        }
    ...
    public function delete(ClientApiRequest $request, Server $server, Schedule $schedule, Task $task): JsonResponse
    {
        ...
        if (!$request->user()->can(Permission::ACTION_SCHEDULE_UPDATE, $server)) {
            throw new HttpForbiddenException(trans('exceptions.tasks.permission_denied'));
        }

        if ($task->is_queued || $task->is_processing) {
            throw new HttpForbiddenException(trans('exceptions.tasks.task_queued'));
        }
