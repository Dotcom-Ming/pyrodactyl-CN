...
class BackupController extends ClientApiController
{
    ...
    public function download(Request $request, Server $server, Backup $backup): JsonResponse
    {
        ...
        if ($backup->disk !== Backup::ADAPTER_AWS_S3 && $backup->disk !== Backup::ADAPTER_WINGS) {
            throw new BadRequestHttpException(trans('exceptions.backup.unknown_disk'));
        }
    ...
    public function restore(RestoreBackupRequest $request, Server $server, Backup $backup): JsonResponse
    {
        ...
        if (!is_null($server->status)) {
            throw new BadRequestHttpException(trans('exceptions.backup.restore_state'));
        }

        if (!$backup->is_successful && is_null($backup->completed_at)) {
            throw new BadRequestHttpException(trans('exceptions.backup.restore_unavailable'));
        }
