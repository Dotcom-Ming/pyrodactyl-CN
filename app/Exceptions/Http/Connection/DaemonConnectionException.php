...
class DaemonConnectionException extends DisplayException
{
    ...
    public function __construct(GuzzleException $previous, bool $useStatusCode = true)
    {
        ...
        if (is_null($response)) {
            $message = trans('strings.daemon_connection_error');
        } else {
            $message = trans('strings.daemon_communication_error') . sprintf(' (code: %s) (request_id: %s)', $response->getStatusCode(), $this->requestId ?? '<nil>');
        }
        ...
        if ($this->statusCode < 500 && !is_null($response)) {
            ...
            $message = sprintf(trans('strings.daemon_remote_error'), $body['error'] ?? $message, $this->requestId ?? '<nil>');
        }
