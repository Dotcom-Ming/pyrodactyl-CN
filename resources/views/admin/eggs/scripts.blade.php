...
@section('title')
    {{ trans('strings.admin_nests') }} &rarr; Egg: {{ $egg->name }} &rarr; {{ trans('strings.admin_install_script') }}
@endsection

@section('content-header')
    <h1>{{ $egg->name }}<small>{{ trans('strings.admin_manage_install_script_desc') }}</small></h1>
    <ol class="breadcrumb">
    ...
    <div class="col-xs-12">
        <div class="nav-tabs-custom nav-tabs-floating">
            <ul class="nav nav-tabs">
                ...
                <li class="active"><a href="{{ route('admin.nests.egg.scripts', $egg->id) }}">{{ trans('strings.admin_install_script') }}</a></li>
            </ul>
        </div>
    </div>
</div>
        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header with-border">
                    <h3 class="box-title">{{ trans('strings.admin_install_script') }}</h3>
                </div>
                @if(! is_null($egg->copyFrom))
                    <div class="box-body">
                        <div class="callout callout-warning no-margin">
                            {!! trans('strings.admin_copy_script_notice', ['name' => '<a href="' . route('admin.nests.egg.view', $egg->copyFrom->id) . '">' . e($egg->copyFrom->name) . '</a>']) !!}
                        </div>
                    </div>
                @endif
                ...
                <div class="box-body">
                    <div class="row">
                        <div class="form-group col-sm-4">
                            <label class="control-label">{{ trans('strings.admin_copy_script_from') }}</label>
                            ...
                            <p class="text-muted small">{{ trans('strings.admin_copy_script_desc') }}</p>
                        </div>
                        <div class="form-group col-sm-4">
                            <label class="control-label">{{ trans('strings.admin_script_container') }}</label>
                            <input type="text" name="script_container" class="form-control" value="{{ $egg->script_container }}" />
                            <p class="text-muted small">{{ trans('strings.admin_script_container_desc') }}</p>
                        </div>
                        <div class="form-group col-sm-4">
                            <label class="control-label">{{ trans('strings.admin_script_entrypoint') }}</label>
                            <input type="text" name="script_entry" class="form-control" value="{{ $egg->script_entry }}" />
                            <p class="text-muted small">{{ trans('strings.admin_script_entrypoint_desc') }}</p>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-12 text-muted">
                            {{ trans('strings.admin_script_rely') }}
                            @if(count($relyOnScript) > 0)
                                ...
                                @endforeach
                            @else
                                <em>{{ trans('strings.admin_none_lower') }}</em>
                            @endif
                        </div>
                    </div>
                </div>
