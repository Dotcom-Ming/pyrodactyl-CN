...
@section('title')
    {{ trans('strings.admin_mounts') }} &rarr; {{ $mount->id }}
@endsection
...
@section('content-header')
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="{{ route('admin.mounts') }}">{{ trans('strings.admin_mounts') }}</a></li>
...
@section('content')
    <div class="row">
        <div class="col-sm-6">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">{{ trans('strings.admin_mount_details') }}</h3>
                </div>

                <form action="{{ route('admin.mounts.view', $mount->id) }}" method="POST">
                    <div class="box-body">
                        <div class="form-group">
                            <label for="PUniqueID" class="form-label">{{ trans('strings.admin_unique_id') }}</label>
        ...
        <div class="col-sm-6">
            <div class="box">
                <div class="box-header with-border">
                    <h3 class="box-title">{{ trans('strings.admin_mount_eggs') }}</h3>

                    <div class="box-tools">
                        <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#addEggsModal">{{ trans('strings.admin_add_eggs') }}</button>
                    </div>
                </div>
            ...
            <div class="box">
                <div class="box-header with-border">
                    <h3 class="box-title">{{ trans('strings.admin_mount_nodes') }}</h3>

                    <div class="box-tools">
                        <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#addNodesModal">{{ trans('strings.admin_add_nodes') }}</button>
                    </div>
                </div>
            ...
            <div class="modal-content">
                <form action="{{ route('admin.mounts.eggs', $mount->id) }}" method="POST">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">{{ trans('strings.admin_add_eggs') }}</h4>
                    </div>

                    <div class="modal-body">
                        <div class="row">
                            <div class="form-group col-md-12">
                                <label for="pEggs">{{ trans('strings.admin_mount_eggs') }}</label>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary btn-sm">{{ trans('strings.admin_add') }}</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="modal-content">
                <form action="{{ route('admin.mounts.nodes', $mount->id) }}" method="POST">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">{{ trans('strings.admin_add_nodes') }}</h4>
                    </div>

                    <div class="modal-body">
                        <div class="row">
                            <div class="form-group col-md-12">
                                <label for="pNodes">{{ trans('strings.admin_mount_nodes') }}</label>
...
@section('footer-scripts')
    ...
    <script>
        $(document).ready(function() {
            $('#pEggs').select2({
                placeholder: @json(trans('strings.admin_select_eggs')),
            });

            $('#pNodes').select2({
                placeholder: @json(trans('strings.admin_select_nodes')),
            });
            ...
            $('button[data-action="detach-egg"]').click(function (event) {
                ...
                $.ajax({
                    ...
                    swal({ type: 'success', title: @json(trans('strings.admin_egg_detached')) });
                }).fail(function (jqXHR) {
                    console.error(jqXHR);
                    swal({
            ...
            $('button[data-action="detach-node"]').click(function (event) {
                ...
                $.ajax({
                    ...
                    element.parent().parent().addClass('warning').delay(100).fadeOut();
                    swal({ type: 'success', title: @json(trans('strings.admin_node_detached')) });
                }).fail(function (jqXHR) {
                    console.error(jqXHR);
                    swal({
                        title: @json(trans('strings.admin_whoops')),
