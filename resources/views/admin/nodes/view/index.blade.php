...
@section('content-header')
  <h1>{{ $node->name }}<small>{{ trans('strings.admin_node_overview_desc') }}</small></h1>
  <ol class="breadcrumb">
...
@section('content')
  <div class="row">
    ...
    <div class="nav-tabs-custom nav-tabs-floating">
      ...
      <li class="active"><a href="{{ route('admin.nodes.view', $node->id) }}">{{ trans('strings.admin_about') }}</a></li>
  ...
  <div class="row">
    ...
    <div class="row">
      ...
      <div class="box box-primary">
        ...
        <table class="table table-hover">
          <tr>
          <td>{{ trans('strings.admin_daemon_version') }}</td>
          <td><code data-attr="info-version"><i class="fa fa-refresh fa-fw fa-spin"></i></code> ({{ trans('strings.admin_latest') }}:
            <code>{{ $version->getDaemon() }}</code>)
          </td>
          </tr>
          <tr>
          <td>{{ trans('strings.admin_system_information') }}</td>
          <td data-attr="info-system"><i class="fa fa-refresh fa-fw fa-spin"></i></td>
          </tr>
          <tr>
          <td>{{ trans('strings.admin_total_cpu_threads') }}</td>
      ...
      <div class="box-header with-border">
      {{ trans('strings.admin_description') }}
      </div>
    ...
    @endif
      ...
      <div class="box box-danger">
        <div class="box-header with-border">
        <h3 class="box-title">{{ trans('strings.admin_delete_node') }}</h3>
        </div>
        <div class="box-body">
        <p class="no-margin">{{ trans('strings.admin_delete_node_desc') }}</p>
        </div>
        <div class="box-footer">
        <form action="{{ route('admin.nodes.view.delete', $node->id) }}" method="POST">
          {!! csrf_field() !!}
          {!! method_field('DELETE') !!}
          <button type="submit" class="btn btn-danger btn-sm pull-right" {{ ($node->servers_count < 1) ?: 'disabled' }}>{{ trans('strings.admin_confirm_delete_node') }}</button>
        </form>
        </div>
      </div>
      </div>
    ...
    <div class="box box-primary">
      <div class="box-header with-border">
      <h3 class="box-title">{{ trans('strings.admin_at_a_glance') }}</h3>
      </div>
      <div class="box-body">
      <div class="row">
      ...
      <div class="info-box bg-orange">
        ...
        <span class="info-box-text">{{ trans('strings.admin_node_under') }}</span>
        <span class="info-box-number">{{ trans('strings.admin_maintenance') }}</span>
        </div>
      </div>
      </div>
      @endif
    ...
    @endphp
        ...
        <div class="info-box bg-{{ $diskColor}}" style="background: {{ $diskColor }}">
          ...
          <span class="info-box-text">{{ trans('strings.admin_disk_allocated') }}</span>
        ...
        <div class="info-box bg-{{ $memoryColor}}" style="background: {{ $memoryColor }}">
          ...
          <div class="info-box-content" style="padding: 15px 10px 0;">
          <span class="info-box-text">{{ trans('strings.admin_memory_allocated') }}</span>
          <span class="info-box-number">
      ...
      <div class="info-box bg-blue">
        ...
        <div class="info-box-content" style="padding: 23px 10px 0;">
        <span class="info-box-text">{{ trans('strings.admin_total_servers') }}</span>
