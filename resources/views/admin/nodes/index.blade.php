...
@section('content-header')
    <h1>{{ trans('strings.admin_nodes') }}<small>{{ trans('strings.admin_all_nodes') }}</small></h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('admin.index') }}">{{ trans('strings.admin') }}</a></li>
        <li class="active">{{ trans('strings.admin_nodes') }}</li>
    </ol>
@endsection
    ...
    <div class="col-xs-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">{{ trans('strings.admin_node_list') }}</h3>
                <div class="box-tools search01">
                    <form action="{{ route('admin.nodes') }}" method="GET">
                        <div class="input-group input-group-sm">
                            <input type="text" name="filter[name]" class="form-control pull-right" value="{{ request()->input('filter.name') }}" placeholder="{{ trans('strings.admin_search_nodes') }}">
                            <div class="input-group-btn">
                                <button type="submit" class="btn btn-default"><i class="fa fa-search"></i></button>
                                <a href="{{ route('admin.nodes.new') }}"><button type="button" class="btn btn-sm btn-primary" style="border-radius: 0 3px 3px 0;margin-left:-1px;">{{ trans('strings.admin_create_node') }}</button></a>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <table class="table table-hover">
                    <tbody>
                        <tr>
                            ...
                            <th>{{ trans('strings.admin_location') }}</th>
                            <th>{{ trans('strings.admin_memory_percent') }}</th>
                            <th>{{ trans('strings.admin_allocated_memory') }}</th>
                            <th>{{ trans('strings.admin_total_memory') }}</th>
                            <th>{{ trans('strings.admin_disk_percent') }}</th>
                            <th>{{ trans('strings.admin_allocated_disk') }}</th>
                            <th>{{ trans('strings.admin_total_disk') }}</th>
                            <th class="text-center">{{ trans('strings.admin_servers') }}</th>
                            <th class="text-center">{{ trans('strings.admin_daemon_type') }}</th>
                            <th class="text-center">{{ trans('strings.admin_public') }}</th>
                        </tr>
    ...
    (function pingNodes() {
        $('td[data-action="ping"]').each(function(i, element) {
            $.ajax({
                ...
                var errorText = '{{ trans('strings.admin_node_connect_error') }}';
                try {
