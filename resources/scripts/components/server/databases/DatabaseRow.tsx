...
const DatabaseRow = ({ database }: Props) => {
    ...
    const schema = object().shape({
        confirm: string()
            ...
            .oneOf([database.name.split('_', 2)[1] || '', database.name], t('server.validation_database_name_required')),
    });
    ...
    return (
        <>
            <Formik onSubmit={submit} initialValues={{ confirm: '' }} validationSchema={schema} isInitialValid={false}>
                {({ isSubmitting, isValid, resetForm }) => (
                    <Modal
                        ...
                        title={t('strings.confirm_database_deletion')}
                    >
                        <FlashMessageRender byKey={'database:delete'} />
                        <div className='flex flex-col'>
                            <p>
                                {t('strings.deleting_database_warning', { name: database.name })}
                            </p>
                            <Form className='mt-6'>
                                <Field
                                    type={'text'}
                                    id={'confirm_name'}
                                    name={'confirm'}
                                    label={t('strings.confirm_database_name')}
                                    description={t('strings.enter_database_name')}
                                />
                                <ActionButton
                                    variant='danger'
                                    type={'submit'}
                                ...
                                >
                                    ...
                                    {isSubmitting ? t('strings.deleting') : t('strings.delete_database')}
                                </ActionButton>
                            </Form>
                        </div>
                    </Modal>
            ...
            <Modal
                ...
                title={t('strings.database_connection_details')}
                closeButton={true}
            ...
            >
                ...
                <div className='flex flex-col min-w-full gap-4'>
                    <div className='grid gap-4 sm:grid-cols-2 min-w-full'>
                        <div className='flex flex-col'>
                            <Label>{t('strings.endpoint')}</Label>
                            <CopyOnClick text={database.connectionString}>
                                <Input type={'text'} readOnly value={database.connectionString} />
                            </CopyOnClick>
                        </div>
                        <div className='flex flex-col'>
                            <Label>{t('strings.connections_from')}</Label>
                    ...
                    <div className='flex flex-col'>
                        <div className='flex flex-row gap-2 align-middle items-center'>
                            <Label>{t('strings.jdbc_connection_string')}</Label>
                        </div>
                ...
                <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 w-full'>
                    ...
                    <div className='flex items-center gap-2 sm:flex-col sm:gap-3'>
                        ...
                        >
                            ...
                            <span className='hidden sm:inline'>{t('strings.details')}</span>
                        </ActionButton>
