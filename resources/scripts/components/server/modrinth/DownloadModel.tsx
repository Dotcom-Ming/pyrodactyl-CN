            ...
            <div className={`relative flex justify-center ${className}`}>
                <div className='relative w-full max-w-md'>
                    <Button
                        ...
                        disabled
                    >
                        <span className='font-medium truncate'>{t('server.no_versions_available')}</span>
                    </Button>
                </div>
            </div>
        );
                ...
                >
                    <div className='flex flex-col'>
                        <span className='font-medium truncate'>
                            {t('server.version_label')}: {selectedVersion?.version_number || t('server.select_a_version')}
                        </span>
                                ...
                                <div className='flex flex-col'>
                                    ...
                                    <div className='flex gap-2 mt-1 text-xs text-gray-400'>
                                        {version.files?.[0]?.file_type && (
                                            <span>
                                                {t('server.file_type')}: {version.files[0].file_type}
                                            </span>
                                        )}
                                        {version.files?.[0]?.size && (
                                            <span>
                                                {t('server.file_size')}: {formatFileSize(version.files[0].size)}
                                            </span>
                                        )}
                                        {version.game_versions?.length > 0 && (
                                            <span>
                                                {t('server.game_version')}: {version.game_versions[0]}
                                            </span>
                                        )}
                                    </div>
                                </div>
